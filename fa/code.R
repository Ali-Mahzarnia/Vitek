...
## BJ sheet
BJsheet  = read.delim('studywide_stats_for_fa.txt')
BJsheet$structure[168:333] = BJsheet$structure[2:167]
dim(BJsheet)
...
write.csv( file = "results_fa.csv"  ,pvalsresults )
...
# pvalsresultsadjusted[,8] = p.adjust(pvalsresultsadjusted[,7], "fdr") #Benjamini & Hochberg
...
for ( jj in 1:dim(sig)[1]) {
  if ( length( grep("NA", rownames(sig)[jj] ) )<1   ){
